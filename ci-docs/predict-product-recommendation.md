...
1. 結果頁面中有五個主要資料區段：
    ...
    以下是可能會影響模型生成產品建議的因素。
        - **過去交易**：過去的購買模式會被模型用來生成產品建議。 例如，如果某個人最近購買了 *Surface Book 3* 和 *Surface Pen*，則模型會建議 *Surface Arc 滑鼠*。 此模型瞭解到過去有許多客戶在購買 *Surface Book 3* 和 *Surface Pen* 後，就購買 *Surface Arc 滑鼠*。
