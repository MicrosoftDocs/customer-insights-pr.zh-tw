---
title: 建立客戶細分
description: 使用客戶細分立器或快速客戶細分，以根據各種屬性來群組客戶並建立客戶細分。
ms.date: 03/25/2022
...
建立客戶細分時，您可以儲存草稿。 在草稿階段中，區段會以非使用中的區段儲存。 當您完成區段組態時，請執行它以啟動區段。 您也可以從 **所有區段** 頁面 **啟動** 某個區段。
...
1. 如果規則有多個條件，您可以選擇用哪個邏輯運算子連接它們。  
   ...
   - 您可以建立多個規則以建立不同的客戶記錄集。 您可以結合群組以包括商務案例中必要的客戶。 若要建立新規則，請選取 **新增規則**。 特別是當您因為具體指明的關係路徑而無法將實體納入規則時，您就必須建立新規則才能選擇屬性表單。
