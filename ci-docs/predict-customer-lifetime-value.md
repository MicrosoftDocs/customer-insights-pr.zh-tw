...
1. 在 **其他資料 (選用)** 步驟中，選取 **使用額外活動資料提升模型深入解析** 底下的 **新增資料**。 選擇客戶活動實體提供[先決條件](#prerequisites)說明的客戶活動資訊。
...
1. 在 **其他資料 (選用)** 步驟中，選取 **使用額外客戶資料提升模型深入解析** 底下的 **新增資料**。

1. 如果是 **實體**，選擇 **Customer : CustomerInsights** 以選擇對應到客戶屬性資料的統一客戶設定檔資料表。 如果是 **客戶識別碼**，選擇 **System.Customer.CustomerId**。
...
1. 在對應模型應該用來幫助預測客戶留存期間值的屬性後，選擇 **儲存**。
