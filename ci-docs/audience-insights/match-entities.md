...
> - 選擇與其他實體有幾項共用屬性的實體 (例如：名稱、電話號碼或電子郵件地址) 為主要實體。

1. 移至 **資料** > **統整** > **比對**，然後選取 **設定順序**，開始比對階段。
1. 選取 **實體順序**。 例如，選取 **eCommerce:eCommerceContacts** 作為主要實體，而 **LoyaltyScheme:loyCustomers** 作為第二實體。 
1. 若要讓實體中的每筆記錄作為唯一客戶，並且與後續每一個實體進行比對，選取 **包含全部**。
1. 選取 **完成**。 

指定符合順序之後，已定義的比對組會顯示在 **資料** > **整合** > **比對** 中的 **比對記錄詳細資料** 區段。 在比對完成之前，關鍵計量是空白的。
...
1. 實體的底下在 **比對記錄詳細資料** 區段，選取 **新增規則** 定義比對規則。
...
1. 在 **建立規則** 的窗格中，設定規則的條件。
   ...
   - **精度**：設定適用此條件的精準度等級。 
     - **基本**：從 *低*、*中*、*高* 與 *完全相符* 進行選擇。 選取 **全字詞**，只比對 100% 相符的記錄。 選取其中一個其他等級，以比對不是 100% 相同的記錄。
...
重複資料刪除記錄被用在跨實體比對程序。 重複資料刪除記錄發生在個別實體上，而且可以對在比對組中使用到的每個實體進行設定。
...
1. 請在 **合併偏好** 窗格中選取您要執行重複資料刪除規則的實體。
   ...
   1. 或者，若要定義實體個別屬性的重複資料刪除規則，請選取 **進階**。 例如，您可以選擇保留最近的電子郵件，「及」來自不同記錄的最完整地址。 展開實體以查看其所有屬性，並定義要用於個別屬性的選項。 如果您選擇基於新近度的選項，也需要指定定義新近度的日期/時間欄位。 
